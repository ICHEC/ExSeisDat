 ...
 */
grid_t get_grid(const Grid item, const unsigned char* buf)
{
    auto p = get_pair(item);
    return grid_t(get_md(p.first, buf), get_md(p.second, buf));
}
 ...
 */
void set_grid(const Grid item, const grid_t grid, unsigned char* buf)
{
    auto pair = get_pair(item);
 ...
 */
int16_t calc_scale(const coord_t coord)
{
    // I get the minimum value so that I definitely store the result.
    // This is at the expense of precision.
    int16_t scal1 = segy::find_scalar(coord.x);
    int16_t scal2 = segy::find_scalar(coord.y);
    return scal_comp(scal1, scal2);
}
...
// {
//     int16_t scale = scal_comp(1, calc_scale(prm->src));
//     scale = scal_comp(scale, calc_scale(prm->rcv));
//     scale = scal_comp(scale, calc_scale(prm->cmp));
//     setScale(TrScal::ScaleCoord, scale, md);
//
//     set_coord(Coord::Src, prm->src, scale, md);
//     set_coord(Coord::Rcv, prm->rcv, scale, md);
//     set_coord(Coord::CMP, prm->cmp, scale, md);
//     set_grid(Grid::Line, prm->line, md);
//
