...
const std::vector<exseis::utils::Trace_value> lowpass_time = {
    0.422621,  0.530021,   0.632801,  0.722758,  0.791212,  0.829939,
    0.832105,  0.793097,   0.711253,  0.588246,  0.429078,  0.241856,
    0.0372786, -0.172197,  -0.373282, -0.552796, -0.698696, -0.801013,
    -0.852533, -0.849352,  -0.791238, -0.681619, -0.527352, -0.338336,
    -0.126833, 0.0933952,  0.30795,   0.502739,  0.664977,  0.783999,
    0.851935,  0.864286,   0.820235,  0.722647,  0.577927,  0.395643,
    0.187832,  -0.0317768, -0.248623, -0.448336, -0.617733, -0.745662,
    -0.823804, -0.847317,  -0.815159, -0.730171, -0.598976, -0.431485,
    -0.240139, -0.0390237, 0.157239,  0.334753,  0.481512,  0.588441,
    0.650331,  0.666335,   0.639985,  0.57875,   0.492984};
...
const std::vector<exseis::utils::Trace_value> bandpass_freq = {
    0.72428,   0.331669,  0.239852,  0.502776,   -0.409657, 0.144657,
    -0.456118, -0.357781, -0.243053, -0.883959,  -0.272044, -1.01206,
    -0.713787, -0.649706, -1.04527,  -0.367378,  -1.07208,  -0.536751,
    -0.47469,  -0.62768,  0.169675,  -0.475295,  0.256944,  0.177763,
    0.213481,  0.904333,  0.235598,  0.900613,   0.534085,  0.50306,
    0.875472,  0.0886262, 0.594238,  -0.0739399, -0.207737, -0.0302071,
    -0.842162, -0.256339, -1.02178,  -0.860615,  -0.682687, -1.23377,
    -0.383137, -0.985877, -0.333746, -0.150852,  -0.392555, 0.543525,
    0.0571215, 0.844476,  0.821583,  0.672036,   1.38692,   0.719453,
    1.23663,   0.986613,  0.81142,   1.25001,    0.325807};
...

{
    for (size_t i = 0; i < nt; i++) {
        ...
        auto* trace_i = &(trc[i * ns]);

        exseis::utils::mute(
            ns, trace_i, taper_function, mute_size_at_begin,
    ...
    size_t taper_size_at_end)
{
    for (size_t i = 0; i < nt; i++) {
        size_t wt_begin = 0;
        size_t wt_end   = taper_size_at_end;
        for (size_t j = 0; j < ns; j++) {
            if ((wt_begin < 1) && (trc[i * ns + j] != 0.0f)) {
                ++wt_begin;
                trc[i * ns + j] =
                    trc[i * ns + j] * func(wt_begin, taper_size_at_begin);
            }
            else if ((wt_begin > 0) && (wt_begin < taper_size_at_begin)) {
                ++wt_begin;
                trc[i * ns + j] *= func(wt_begin, taper_size_at_begin);
            }
            else if ((ns - j) <= taper_size_at_end) {
                trc[i * ns + j] *= func(wt_end, taper_size_at_end);
                --wt_end;
            }
        }
    }
...
TEST_F(SetTest, Mute)
{
    for (auto taper_function :
        ...
        mute_test(100, 200, taper_function, 10, 20, 20, 10);
    }
}
...
TEST_F(SetTest, Taper2TailCos)
{
    taper_test(
        100, 200,
        [](exseis::utils::Trace_value wt,
           exseis::utils::Trace_value ramp) -> exseis::utils::Trace_value {
            const float pi = M_PI;
...
{
    auto agc_func = [](size_t window, exseis::utils::Trace_value* trc,
                       size_t win_cntr) {
        exseis::utils::Trace_value amp = 0.0f;
