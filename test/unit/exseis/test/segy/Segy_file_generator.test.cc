...
TEST_CASE("Text_header", "[test][segy][Segy_file_generator][Text_header]")
{
    ...
    REQUIRE(text_header.size() == exseis::segy::segy_text_header_size());

    SECTION ("Text_header::Native") {
        auto th_native = text_header.native();

        REQUIRE(
            th_native.ascii.size() == exseis::segy::segy_text_header_size());
        REQUIRE(([&] {
    ...
    SECTION ("Text_header::Binary") {
        ...
        REQUIRE(th_binary.size() == exseis::segy::segy_text_header_size());
        REQUIRE(([&] {
...
TEST_CASE("Trace", "[test][segy][Segy_file_generator][Trace]")
{
...

    {
        ...
        for (int16_t i = 0; i < samples_per_trace; i++) {
            ...
            const auto binary_converted =
                exseis::from_ibm_to_float(binary_be, true);

            REQUIRE(
                exseis::to_big_endian(native)
                == exseis::to_big_endian(binary_converted));
        }
    }
    {
        ...
        for (int16_t i = 0; i < samples_per_trace; i++) {
            ...
            REQUIRE(
                exseis::to_big_endian(native_sample)
                == exseis::to_big_endian(samples_pattern_ieee));
        }
    }

    {
        ...
        if (number_encoding == exseis::segy::Segy_number_format::IBM_fp32) {
            ...
            for (int16_t i = 0; i < samples_per_trace; i++) {
                ...
                const float converted_sample = ([&] {
                    switch (number_encoding) {
                        case exseis::segy::Segy_number_format::IBM_fp32:
                            return exseis::from_ibm_to_float(
                                sample_binary_be, true);
                        case exseis::segy::Segy_number_format::IEEE_fp32:
                            return exseis::from_big_endian<float>(
                                sample_binary_be);
                ...
                REQUIRE(
                    exseis::to_big_endian(converted_sample)
                    == exseis::to_big_endian(native_sample));
            }
        }
    }
...
TEST_CASE("Segy_file", "[test][segy][Segy_file_generator][Segy_file]")
{
    ...
    auto number_encoding =
        GENERATE(exseis::segy::Segy_number_format::IEEE_fp32);
