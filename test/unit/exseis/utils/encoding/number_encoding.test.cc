...
TEST_CASE("from_big_endian", "[number_encoding]")
{
    ...
    REQUIRE(exseis::from_big_endian<uint32_t>(a, b, c, d) == native_uint32);
    ...
    REQUIRE(exseis::from_big_endian<int32_t>(a, b, c, d) == native_int32);


    // Check conversion to float
    // Careful of float comparison with '=', the compiler complains
    const float float_from_be     = exseis::from_big_endian<float>(a, b, c, d);
...
TEST_CASE("from_ibm_to_float", "[number_encoding]")
{
    ...
    for (uint32_t significand = 0x100000U; significand <= 0xFFFFFFU;
        ...
        const float ieee = exseis::from_ibm_to_float(ibm_float_be, true);
