...
TEST_CASE("Character Encoding", "[utils][EBCIDIC][ASCII]")
{
    ...
    SECTION ("Translate between test ASCII and EBCDIC strings") {
        ...
        for (size_t i = 0; i < strlen; i++) {
            ...
            REQUIRE(ebcdic_c == exseis::to_ebcdic_from_ascii(ascii_c));
            REQUIRE(ascii_c == exseis::to_ascii_from_ebcdic(ebcdic_c));
        }
    }
    ...
    SECTION ("is_printable_ascii") {
        ...
        for (size_t i = 0; i < 256; i++) {
            ...
            if (i >= 32 && i <= 126) {
                REQUIRE(exseis::is_printable_ascii(ascii_c) == true);
            }
            else {
                REQUIRE(exseis::is_printable_ascii(ascii_c) == false);
            }
        }
    }
    ...
    SECTION ("is_printable_ebcdic <-> is_printable_ascii") {
        ...
        for (size_t i = 0; i < 256; i++) {
            const char ascii_c = static_cast<char>(i);
            const unsigned char ebcdic_c =
                exseis::to_ebcdic_from_ascii(ascii_c);

            REQUIRE(
                exseis::is_printable_ascii(ascii_c)
                == exseis::is_printable_ebcdic(ebcdic_c));
        }
    }
    ...
    SECTION ("ASCII <-> EBCDIC conversion consistency") {
        ...
        for (size_t i = 0; i < 256; i++) {
            const char ascii_c            = static_cast<char>(i);
            const char round_trip_ascii_c = exseis::to_ascii_from_ebcdic(
                exseis::to_ebcdic_from_ascii(ascii_c));

            const unsigned char ebcdic_c = static_cast<unsigned char>(i);
            const unsigned char round_trip_ebcdic_c =
                exseis::to_ebcdic_from_ascii(
                    exseis::to_ascii_from_ebcdic(ebcdic_c));
            ...
            if (i == 0) {
                ...
                REQUIRE(ascii_c == 0);
                REQUIRE(exseis::to_ebcdic_from_ascii(ascii_c) == 0);
                REQUIRE(ebcdic_c == 0);
                REQUIRE(exseis::to_ascii_from_ebcdic(ebcdic_c) == 0);
            ...
            if (exseis::is_printable_ascii(ascii_c)) {
                REQUIRE(ascii_c == round_trip_ascii_c);
            }
            if (exseis::is_printable_ebcdic(ebcdic_c)) {
                REQUIRE(ebcdic_c == round_trip_ebcdic_c);
            }

            // Anything that maps forward should map back
            if (exseis::to_ebcdic_from_ascii(ascii_c) != ebcdic_sub) {
                REQUIRE(ascii_c == round_trip_ascii_c);
            }
            if (exseis::to_ascii_from_ebcdic(ebcdic_c) != ascii_sub) {
                REQUIRE(ebcdic_c == round_trip_ebcdic_c);
            }
        }
    }
