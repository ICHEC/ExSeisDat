#ifndef EXSEIS_TEST_PIOL_BINARY_FILE_TEST_HH
#define EXSEIS_TEST_PIOL_BINARY_FILE_TEST_HH

#include "exseis/piol/io_driver/IO_driver.hh"
#include "exseis/utils/decomposition/block_decomposition.hh"

#include "exseis/test/Buffer.hh"
        ...
        auto decomposition = ([=] {
            switch (access) {
                ...
                case Access::parallel:
                    return exseis::block_decomposition(
                        size, number_of_ranks, rank);

                // single access: only rank 0 allocates and reads a buffer.
                case Access::single:
                    if (rank == 0) {
                        return exseis::Contiguous_decomposition{0, size};
                    }
                    return exseis::Contiguous_decomposition{0, 0};
            }
    ...
    SECTION ("Strided access") {
        ...
        auto block_index_decomposition = ([=] {
            switch (access) {
                ...
                case Access::single:
                    if (rank == 0) {
                        return exseis::Contiguous_decomposition{
                            0, number_of_blocks};
                    }
                    return exseis::Contiguous_decomposition{0, 0};
            }
...
void test_io_driver_2gb_limit(
    exseis::IO_driver& io_driver, const Communicator& communicator)
{
...
#endif  // EXSEIS_TEST_PIOL_BINARY_FILE_TEST_HH
