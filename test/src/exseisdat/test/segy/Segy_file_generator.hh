#ifndef EXSEISDAT_TEST_SEGY_FILE_GENERATOR
#define EXSEISDAT_TEST_SEGY_FILE_GENERATOR
...
class Text_header {
  public:
    ...
    struct Native {
        std::array<unsigned char, exseis::piol::segy::segy_text_header_size()>
            ascii;
        Encoding encoding;
    };
  ...
  public:
    ...
    static unsigned char ascii_pattern(uint64_t i)
    {
        ...
        while (!exseis::utils::is_printable_ascii(ascii)) {
    ...
    {
        return exseis::utils::to_ebcdic_from_ascii(ascii_pattern(i));
    }
...
class Binary_header {
  public:
    struct Native {
        ...
        exseis::piol::segy::Segy_number_format number_format;
    };

    using Binary = std::array<
        unsigned char,
        exseis::piol::segy::segy_binary_file_header_size()
  ...
  public:
    ...
    Binary_header(
        ...
        m_binary{}
    {
        using Offset = exseis::piol::segy::Segy_file_header_byte;
  ...
  private:
    ...
    NativeValue write_header(NativeValue value, size_t offset)
    {
        ...
        const auto value_be =
            exseis::utils::to_big_endian(static_cast<Value>(value));
        std::copy(
...
class Trace {
  ...
  private:
    ...
    template<typename T>
    T hash(exseis::piol::segy::Trace_header_offsets offset) const
    {
    ...
    void init_metadata()
    {
        using Offsets = exseis::piol::segy::Trace_header_offsets;
    ...
    Value write_header(
        Value value, exseis::piol::segy::Trace_header_offsets offset)
    {
        const auto value_be = exseis::utils::to_big_endian(value);
        std::copy(
            value_be.cbegin(), value_be.cend(),
    ...
    template<typename Value>
    Value write_header(exseis::piol::segy::Trace_header_offsets offset)
    {
            ...
            auto sample_binary_be = ([&] {
                switch (m_native.number_format) {
                    case exseis::piol::segy::Segy_number_format::IEEE_fp32:
                        return exseis::utils::to_big_endian(
                            samples_pattern_ieee(i));
...
#endif  // EXSEISDAT_TEST_SEGY_FILE_GENERATOR
