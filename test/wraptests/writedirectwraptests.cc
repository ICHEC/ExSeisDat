  ...
  std::shared_ptr<ExSeis*> piol, std::shared_ptr<Param*> param)
{
    ...
    EXPECT_CALL(
      mockWriteDirect(), ctor(_, GetEqDeref(piol), "Test_WriteDirect_filename"))
    ...
    EXPECT_CALL(
      mockWriteDirect(), writeInc(EqDeref(write_direct_ptr), DoubleEq(720.0)));
    ...
    EXPECT_CALL(
      mockWriteDirect(), writeParamNonContiguous(
    ...
    EXPECT_CALL(
      mockWriteDirect(),
      writeTrace(EqDeref(write_direct_ptr), 760, 770, _, EqDeref(param)))
    ...
    EXPECT_CALL(
      ...
      .WillOnce(WithArgs<2, 3>(Invoke([](const size_t* offsets, float* trace) {
