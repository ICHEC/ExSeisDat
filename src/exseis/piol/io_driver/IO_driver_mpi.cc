...
#include "exseis/utils/mpi_utils/Safe_collective_block_chunks.hh"
#include "exseis/utils/mpi_utils/detected_mpi_implementation.hh"
...
class IO_driver_mpi_detail {
  public:
    ...
    const Communicator_mpi& m_communicator;
    MPI_File m_file;
    MPI_Info m_info;
    ...
    Source_position source_position)
{
    ...
    formatted_message += ": " + mpi_error_to_string(mpi_error, mpi_status);

    m_communicator.log()->add_entry(
        Log_entry{Status::Error, std::move(formatted_message), Verbosity::none,
...
void IO_driver_mpi_detail::free_and_reset_view(MPI_Datatype& view)
{
    ...
    if (err != MPI_SUCCESS) {
        log_error(
            ...
            EXSEIS_SOURCE_POSITION(function_name));
    }
    ...
    m_max_io_chunk_size(options.max_io_chunk_size)
{
    ...
    auto detail =
        IO_driver_mpi_detail{m_file_name, m_communicator, m_file, m_info};
    ...
    err = MPI_File_open(
        m_communicator.mpi_communicator(), m_file_name.c_str(),
        static_cast<int>(mode), m_info, &m_file);
    if (err != MPI_SUCCESS) {
        detail.log_error(
...
std::string IO_driver_mpi::Implementation::file_name() const
{
    return m_file_name;
}
...
size_t IO_driver_mpi::Implementation::get_file_size() const
{
    ...
    MPI_Allreduce(
        &fsz, &max_fsz, 1, mpi_type<MPI_Offset>(), MPI_MAX,
    ...
    const char* function_name)
{
    ...
    const auto block_chunks = mpi_utils::Safe_collective_block_chunks(
        stride_size, size, m_max_io_chunk_size, m_communicator);
...
{
    int max_size = mpi_max_array_length<char>();
    ...
    const char* function_name)
{
    ...
    MPI_Allreduce(
        MPI_IN_PLACE, &global_max_block_size, 1, mpi_type<size_t>(), MPI_MAX,
        m_communicator.mpi_communicator());
    if (global_max_block_size >= m_max_io_chunk_size) {
        const auto stride_chunks = mpi_utils::Safe_collective_block_chunks(
            1, number_of_blocks, 1, m_communicator);
    ...
    const auto stride_chunks = mpi_utils::Safe_collective_block_chunks(
        stride_size, number_of_blocks, m_max_io_chunk_size, m_communicator);
    ...
    const char* function_name)
{
    ...
    MPI_Allreduce(
        ...
        m_communicator.mpi_communicator());
    if (global_max_block_size >= m_max_io_chunk_size) {
        const auto offset_chunks = mpi_utils::Safe_collective_block_chunks(
            1, number_of_blocks, 1, m_communicator);
...
Communicator IO_driver_mpi::Implementation::communicator() const
{
