...
#warning get rid of this PI post-merge
const trace_t PI = std::acos(-1);
const cmtrace_t I(0,1);
...
void makeFilter(FltrType type, trace_t * numer, trace_t * denom, llint N, trace_t fs, trace_t cf1, trace_t cf2)
{
    ...
    for (llint i = 0; i < N; i++)
        p[i] = -exp(I*PI*trace_t(1LL+2LL*i-N)/trace_t(2LL*N));
    trace_t k;
    switch (type)
    {
        default:
...
void filterTime(size_t nw, trace_t * trcOrgnl, size_t numTail, trace_t * numer, trace_t * denom, FltrPad padding)
{
    ...
    for (size_t i = 0; i < numTail; i++)
        ziF[i] = zi[i]*trcX[0];
    IIR(numTail, nw+6LU*(numTail+1LU), numer, denom, trcX.data(), trcY.data(), ziF.data());
    ...
    for (size_t i = 0; i < numTail; i++)
        zi[i] *= trcX[0];
    IIR(numTail, nw+6LU*(numTail+1LU), numer, denom, trcX.data(), trcY.data(), zi.data());
    for (size_t i = 0; i < nw; i++)
...
void temporalFilter(size_t nt, size_t ns, trace_t * trc, trace_t fs, FltrType type, FltrDmn domain, PadType pad, size_t nw, size_t winCntr, std::vector<trace_t> corners, size_t N)
{
    ...
    for (size_t i = 0; i < nt; i++)
    {
        ...
        {
            case FltrDmn::Time :
                filterTime(nw, trcOrgnl.data(), numTail, numer.data(), denom.data(), getPad(pad));
            break;
            case FltrDmn::Freq :
                filterFreq(nw, trcOrgnl.data(), fs, numTail,  numer.data(), denom.data(), getPad(pad));
            break;
        }
        for (size_t j = 0; j < nw; j++)
