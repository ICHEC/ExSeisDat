  ...
  trace_t normR)
{
    ...
    for (size_t i = 0LU; i < nt; i++) {
        for (size_t j = 0LU; j < win2 + 1LU; j++) {
            ...
            trcAGC[j + ns - win2] = func(
              &trc[(i + 1LU) * ns - 2LU * win2 + j], 2LU * win2 - j, normR,
              win2);
        }
...
trace_t AGCRMS(trace_t* trc, size_t window, trace_t normR, llint)
{
    ...
    size_t num = std::count_if(
      trc, &trc[window], [](trace_t i) { return i != trace_t(0); });
...
{
    trace_t winFullTail =
      std::max(std::abs(winCntr), std::abs(llint(window) - winCntr - 1L));
    trace_t amp = trace_t(0);
    for (llint j = 0LU; j < static_cast<llint>(window); j++)
        amp += pow(
          trc[j] * (trace_t(1) - std::abs(trace_t(j - winCntr) / winFullTail)),
          trace_t(2));
    size_t num = std::count_if(
      trc, &trc[window], [](trace_t i) { return i != trace_t(0); });
    return normR / std::sqrt(amp / trace_t(!num ? 1LU : num));
...
trace_t AGCMedian(trace_t* trc, size_t window, trace_t normR, llint)
{
    ...
    return normR
           / (window % 2LU == 0LU ?
                ((trcTmp[window / 2LU] + trcTmp[window / 2LU + 1LU])
                 / trace_t(2)) :
                trcTmp[window / 2LU]);
}
