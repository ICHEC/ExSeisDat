...
SEGY::SEGY(std::shared_ptr<ExSeisPIOL> piol_, std::string name_, const SEGY::Opt &, std::shared_ptr<Data::Interface> data_, FileMode) : Interface(piol_, name_, data_)
{
}

SEGY::SEGY(std::shared_ptr<ExSeisPIOL> piol_, std::string name_, std::shared_ptr<Data::Interface> data_, FileMode) : Interface(piol_, name_, data_)
{
}
...
{
    data_->read(SEGSz::getDOLoc(offset, ns), SEGSz::getMDSz(), SEGSz::getDOSz(ns), sz, md);
}

void SEGY::writeDOMD(const size_t offset, const size_t ns, const size_t sz, const uchar * md) const
{
    data_->write(SEGSz::getDOLoc(offset, ns), SEGSz::getMDSz(), SEGSz::getDOSz(ns), sz, md);
}

void SEGY::readDODF(const size_t offset, const size_t ns, const size_t sz, uchar * df) const
{
    data_->read(SEGSz::getDODFLoc(offset, ns), SEGSz::getDFSz(ns), SEGSz::getDOSz(ns), sz, df);
}

void SEGY::writeDODF(const size_t offset, const size_t ns, const size_t sz, const uchar * df) const
{
    data_->write(SEGSz::getDODFLoc(offset, ns), SEGSz::getDFSz(ns), SEGSz::getDOSz(ns), sz, df);
}
