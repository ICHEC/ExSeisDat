...
int16_t find_scalar(exseis::utils::Floating_point value)
{
    ...
    exseis::utils::Integer integer_part = exseis::utils::Integer(value);
    ...
    exseis::utils::Integer digits =
        std::llround(value * exseis::utils::Floating_point(tenk))
        - integer_part * tenk;
    ...
    if (digits != 0) {
        ...
        for (int16_t i = 1; i < tenk; i *= 10) {
            if ((digits % (i * 10)) != 0) {
                ...
                // byte
                exseis::utils::Floating_point scal = parse_scalar(scale_factor);

                // int32_t t = exseis::utils::Integer(value / scal) -
                // digits;
