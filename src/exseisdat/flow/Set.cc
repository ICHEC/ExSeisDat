...
typedef std::function<void(
    const detail::Trace_block* in, detail::Trace_block* out)>
    Mod;

/// Typedef for functions that have the same input and output of
/// traces/parameters
typedef std::function<std::vector<size_t>(detail::Trace_block* data)>
    InPlaceMod;
...
{
    m_file.emplace_back(std::make_shared<detail::File_descriptor>());
    auto& f = m_file.back();
    f->ifc  = std::move(in);
...
void Set::add(std::string pattern)
{
    ...
    for (size_t i = 0; i < globs.gl_pathc; i++) {
        ...
        if (std::regex_match(globs.gl_pathv[i], reg)) {
            add(std::make_unique<Input_file_segy>(m_piol, globs.gl_pathv[i]));
        }
    }
    globfree(&globs);
    m_piol->assert_ok();
...
std::unique_ptr<detail::Trace_block> Set::calc_func(
    ...
    std::unique_ptr<detail::Trace_block> b_in)
{
    ...
    if (type == detail::Function_options::Gather) {
        if ((*f_curr)->opt.check(detail::Function_options::Gather)) {
            auto b_out = std::make_unique<detail::Trace_block>();
            dynamic_cast<detail::Operation<Mod>*>(f_curr->get())
    ...
    if (type == detail::Function_options::Gather
        || type == detail::Function_options::SingleTrace) {
        if ((*f_curr)->opt.check(detail::Function_options::SingleTrace)) {
            type = detail::Function_options::SingleTrace;
            dynamic_cast<detail::Operation<InPlaceMod>*>(f_curr->get())
                ->func(b_in.get());
        }
    }
    ...
    FuncLst::iterator f_curr, const FuncLst::iterator f_end)
{
    ...
    for (auto& o : m_fmap) {
        ...
        Output_file_segy out(m_piol, name);
        ...
        for (auto& f : f_que) {
            ...
            for (size_t i = 0; i < lnt; i += max) {
                ...
                auto b_in = std::make_unique<detail::Trace_block>();
                ...
                auto b_final = calc_func(
                    f_curr, f_end, detail::Function_options::SingleTrace,
                    std::move(b_in));
            ...
            for (size_t i = 0; i < extra; i++) {
                ...
                auto b_final = calc_func(
                    f_curr, f_end, detail::Function_options::Gather,
                    std::move(b_in));
                out.write_trace_non_contiguous(
...
{
    if (m_file.size() > 1LU) {
        detail::Operation_options opt = {detail::Function_options::NeedMeta,
                                         detail::Function_options::NeedTrcVal,
                                         detail::Function_options::SingleTrace};
        FuncLst t_func;
        t_func.push_back(std::make_shared<detail::Operation<InPlaceMod>>(
            opt, nullptr, nullptr,
            [](detail::Trace_block*) -> std::vector<size_t> {
    ...
    for (auto& o : m_file) {
        ...
        for (auto f_temp = f_curr;
             f_temp != f_end
             && (*f_temp)->opt.check(detail::Function_options::Gather);
             f_temp++) {
            auto* p = dynamic_cast<detail::Operation<Mod>*>(f_temp->get());
            assert(p);
        ...
        //       functions
        const auto gather_rule = Rule(std::initializer_list<Trace_metadata_key>{
        ...
        Output_file_segy out(m_piol, gname);
    ...
    while (++f_curr != f_end
           && (*f_curr)->opt.check(detail::Function_options::Gather)) {
    }
...
{
    std::shared_ptr<detail::Trace_block> block;

    if ((*f_curr)->opt.check(detail::Function_options::NeedMeta)) {
        if (!(*f_curr)->opt.check(detail::Function_options::NeedTrcVal)) {
    ...
    else if ((*f_curr)->opt.check(detail::Function_options::NeedTrcVal)) {
    ...
    if (++f_curr != f_end) {
        if ((*f_curr)->opt.check(detail::Function_options::SubSetOnly)) {
    ...
    FuncLst::iterator f_curr, const FuncLst::iterator f_end)
{
    if (f_curr != f_end) {
        if ((*f_curr)->opt.check(detail::Function_options::SubSetOnly)) {
        ...
        else if ((*f_curr)->opt.check(detail::Function_options::Gather)) {
            ...
            // Skip fCurr where check(detail::Function_options::Gather) is ok
        ...
        else if ((*f_curr)->opt.check(detail::Function_options::SingleTrace)) {
...
std::vector<std::string> Set::output(std::string oname)
{
    detail::Operation_options opt = {detail::Function_options::NeedMeta,
                                     detail::Function_options::NeedTrcVal,
                                     detail::Function_options::SingleTrace};
    m_func.emplace_back(std::make_shared<detail::Operation<InPlaceMod>>(
        opt, nullptr, nullptr, [](detail::Trace_block*) -> std::vector<size_t> {
            return std::vector<size_t>{};
        }));
...
{
    auto r = std::make_shared<Rule>(std::initializer_list<Trace_metadata_key>{
        Trace_metadata_key::il, Trace_metadata_key::xl,
        ...
        Trace_metadata_key::Offset, Trace_metadata_key::WtrDepRcv,
...
{
    detail::Operation_options opt = {detail::Function_options::NeedMeta,
                                     detail::Function_options::ModMetaVal,
                                     detail::Function_options::DepMetaVal,
                                     detail::Function_options::SubSetOnly};

    m_func.push_back(std::make_shared<detail::Operation<InPlaceMod>>(
        opt, r, nullptr,
        [this, sort_func](detail::Trace_block* in) -> std::vector<size_t> {
    ...
    exseis::utils::Floating_point output_sample_interval)
{
    detail::Operation_options opt = {detail::Function_options::NeedMeta,
                                     detail::Function_options::NeedTrcVal,
                                     detail::Function_options::ModTrcVal,
                                     detail::Function_options::ModMetaVal,
                                     detail::Function_options::DepTrcVal,
                                     detail::Function_options::DepTrcOrder,
                                     detail::Function_options::DepTrcCnt,
                                     detail::Function_options::DepMetaVal,
                                     detail::Function_options::Gather};
    auto state = std::make_shared<detail::Radon_gather_state>(
        m_piol, vm_name, v_bin, output_traces_per_gather,
        output_sample_interval);
    m_func.emplace_back(std::make_shared<detail::Operation<Mod>>(
        opt, m_rule, state,
        [state](const detail::Trace_block* in, detail::Trace_block* out) {
            ...
            for (size_t j = 0; j < state->output_traces_per_gather; j++) {
                ...
                out->prm->set_integer(
                    j, Trace_metadata_key::il, state->il[in->gather_number]);
                out->prm->set_integer(
                    j, Trace_metadata_key::xl, state->xl[in->gather_number]);
            }
        }));
}
...
{
    detail::Operation_options opt = {detail::Function_options::NeedTrcVal,
                                     detail::Function_options::ModTrcVal,
                                     detail::Function_options::DepTrcVal,
                                     detail::Function_options::SingleTrace};
    m_func.emplace_back(std::make_shared<detail::Operation<InPlaceMod>>(
        opt, m_rule, nullptr,
        [=](detail::Trace_block* in) -> std::vector<size_t> {
    ...
    exseis::utils::Trace_value target_amplitude)
{
    ...
    m_func.emplace_back(std::make_shared<detail::Operation<InPlaceMod>>(
        opt, m_rule, nullptr,
        [agc_func, window,
         target_amplitude](detail::Trace_block* in) -> std::vector<size_t> {
    ...
    size_t win_cntr)
{
    ...
    m_func.emplace_back(std::make_shared<detail::Operation<InPlaceMod>>(
        ...
        [type, domain, corners, pad, fs, nw,
         win_cntr](detail::Trace_block* in) -> std::vector<size_t> {
            utils::temporal_filter(
    ...
    size_t win_cntr)
{
    ...
    m_func.emplace_back(std::make_shared<detail::Operation<InPlaceMod>>(
        ...
        [type, domain, corners, pad, fs, nw, win_cntr,
         n](detail::Trace_block* in) -> std::vector<size_t> {
            utils::temporal_filter(
                in->prm->size(), in->ns, in->trc.data(), fs, type, domain, pad,
