...
typedef std::function<void(const TraceBlock* in, TraceBlock* out)>
  Mod;  //!< Typedef for functions that have separate input and output of traces/parameters
typedef std::function<std::vector<size_t>(TraceBlock* data)>
  InPlaceMod;  //!< Typedef for functions that have the same input and output of traces/parameters
