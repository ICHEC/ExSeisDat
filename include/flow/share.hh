...
struct TraceBlock
{
    ...
    std::unique_ptr<File::Param> prm;   //!< unique pointer to parameter structure (if applicable)
    std::vector<trace_t> trc;           //!< traces (if applicable)
};

typedef std::deque<std::shared_ptr<FileDesc>> FileDeque; //!< A typedef to simplify the list of file descriptors
}
#endif
