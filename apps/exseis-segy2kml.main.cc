...
void add_line(
    std::ofstream& file,
    std::string name,
    std::vector<exseis::CoordElem> coords)
{
    file
        << "<Placemark>\n\t<name>" + name
 ...
 *  @param[in] communicator  The communicator to read the file over
 *  @param[in] iname Input file
 *  @param[in] minmax Minimum and maximum coordinates
 */
void calc_min(
    const exseis::Communicator_mpi& communicator,
    std::string iname,
    std::vector<exseis::CoordElem>& minmax)
{
