...
/// @brief Return a fixed-width string representing the given exseis::Trace_metadata_key key at the
///        given trace index from the given exseis::Trace_metadata object.
...
std::string formatted_meta_value(
    ...
    exseis::Trace_metadata_key meta);
...
int main(int argc, char* argv[])
{
    ...
    std::vector<exseis::Trace_metadata_key> metas;
    ...
    const exseis::Input_file_segy input_file(exseis::IO_driver_mpi{
        communicator, input_filename, exseis::File_mode_mpi::Read});
    exseis::IO_driver_mpi output_file(
    ...
    const auto decomposition = exseis::block_decomposition(
        global_number_of_traces, communicator.get_num_rank(),
    ...
    const size_t global_number_of_chunks =
    ...
    exseis::Trace_metadata_key meta)
{
    ...
    switch (trace_metadata.entry_type(meta)) {
        ...
        case exseis::Type::UInt16:
        case exseis::Type::Int8:
        case exseis::Type::UInt8: {
            auto size = std::snprintf(
